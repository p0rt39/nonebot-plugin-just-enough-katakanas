...
class English2KatakanaEngine:
    ...
    def lookup_in_dict(self, word: str, effdict: dict[str, list[str]]) -> list[str]:
        ...
        if not effdict:
            return []  # Dictionary not available, return empty

        return effdict.get(normalized_word, [])
    ...
    def convert_word(self, word: str) -> tuple[str, list[str]]:
        ...
        if results := self.lookup_in_dict(normalized_word, self.ktkn_dict):
...
eng2ktkn_engine = English2KatakanaEngine()
